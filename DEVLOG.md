...
## J132 — Lecture tactile des passifs hors combat

- Les passifs du wiki, de l’inventaire et du gestionnaire d’équipe ouvrent désormais une infobulle lisible au toucher.
- Une seule infobulle reste ouverte à la fois et une nouvelle carte remplace la précédente.
- Les descriptions longues peuvent défiler et l’appui sur l’infobulle la referme sans sélectionner la carte.
...
## J134 — Identité visuelle StickVerse et finition responsive (11 septembre 2026)

- Création d’un socle graphique bleu-noir / turquoise partagé : couleurs,
  formulaires, boutons, focus clavier, notifications et surfaces. Styles séparés
  entre fondations, cartes et compositions de pages ; animations réduites selon
  la préférence système.
- Nouvelle navigation mobile dépliable (Échap, clic extérieur, liens toujours
  disponibles sans JavaScript), lien d’évitement et un seul contenu principal.
- Accueil orienté jeu : vraies cartes de plusieurs raretés, bouton Jouer,
  présentation collection / équipe / combat / progression et espace du joueur.
- L’offre annoncée correspond au code actuel : cinq caisses de départ. Aucun
  ajout de quatre cartes ni changement des récompenses, prix ou probabilités.
- Cartes portrait, rareté explicite, puissance, trio PV / ATQ / DÉF, quantités
  et indication des cartes équipées. Passifs nommés, descriptions défilables,
  une seule fenêtre à la fois et retour du focus après fermeture.
- Wiki et inventaire : recherche insensible aux accents, filtres de rareté
  et de collection, compteur et état vide. Fonctionnement progressif sans JS.
- Fiches Wiki, boutique, probabilités, progression des collections, actualités,
  profil, récompenses, administration et écrans de gestion du compte harmonisés.
- Équipes lisibles en deux colonnes sur téléphone ; rappel explicite des groupes
  X = A + B et Y = C + D. Moteur, sélection, prévisions et animations de combat
  conservés, ainsi que l’ouverture animée des caisses.
- Libellés de présentation « Assassin », « Archer II », « Arbalétrier II » :
  aucun changement des noms stockés, slugs ou relations Doctrine.
- Aucun système amis/messages n’existe dans cette version : aucun faux lien ni
  contenu social ajouté. Aucune migration et aucune modification de sécurité.
- Validation : 298 tests PHPUnit / 2 426 assertions ; 68 templates Twig et
  conteneur de production valides ; compilation AssetMapper validée après
  reconstruction des caches.
- Audit Chrome automatisé reproductible : 29 pages sur 375, 390, 430, 768,
  1024, 1440 et 1920 px (203 rendus), navigation, filtres, mot de passe,
  passifs, composition d’équipe, huit cartes de combat et fenêtre des caisses.
  Aucun débordement horizontal de page, aucune erreur JavaScript ni ressource
  manquante dans ces scénarios. Les fixtures sont créées dans la base de test
  puis annulées ; aucun achat ni compte de production utilisé.
- Tests ajoutés : contrat visuel des pages, stabilité des noms affichés et
  `tests/browser/visual-audit.cjs`. Rendus et rapport générés dans `var/ui-audit`
  (non versionnés). `config/reference.php` reste hors de cette livraison.
